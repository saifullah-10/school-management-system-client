...
export default function AttendancePage() {
  ...
  const dates = useMemo(() => {
    const allDates = Array.from(new Set(records.flatMap(record => Object.keys(record.attendance || {}))));
    // Sort dates in ascending order
    return allDates.sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
  }, [records]);

  const courses = records.length > 0
    ? Array.from(new Set(Object.values(records[0].attendance || {}).flatMap(courseData => Object.keys(courseData))))
    : [];
